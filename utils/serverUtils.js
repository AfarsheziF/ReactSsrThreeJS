...
const serverUtils = {
    ...
    emptyDir(pathFromRoot, skip) {
        return new Promise((resolve, reject) => {
            logger.log('Emptying dir: ' + pathFromRoot);
            skip = skip instanceof Array ? skip : [skip];
            fs.readdir(global.rootDir + pathFromRoot, (err, fileNames) => {
                if (err) reject(err);
                else {
                    if (fileNames.length > 0) {
                        let errors = [], counter = 0;
                        for (const fileName of fileNames) {
                            if (skip.indexOf(fileName) < 0) {
                                fs.unlink(path.join(global.rootDir + pathFromRoot, fileName), (err2) => {
                                    counter++;
                                    if (err2) errors.push(err2);
                                    else logger.log(`File deleted: ${fileName}. ${counter} / ${fileNames.length}`);
            ...
            https.get(uri,
                function (res) {
                    ...
                    file.on('finish', function () {
                        ...
                        file.close(function () {
                            if (readBack) {
                                serverUtils.readFile(pathFromRoot + '.' + format, readEncoding).then(
                                    function (_file) {
                                        resolve(_file);
                                    }, function (e) {
